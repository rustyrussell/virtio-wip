...
return some memory to the host.
...
VIRTIO_MEMBALLOON_HCMD_MIN_BALLOON command from the device via the
fromdevq.
...
The driver replaces the buffer in fromdevq when it is used.

\begin{description}
\item[VIRTIO_MEMBALLOON_GCMD_GET_PAGES (0)] Attempt to get one or more pages out of the balloon (todevq).
...
\begin{enumerate}
\item The driver constructs a buffer of two device-writable le64 values,
...
The driver SHOULD give pages to the balloon when the balloon size is
...
The driver MUST NOT ask for pages which are already available to
